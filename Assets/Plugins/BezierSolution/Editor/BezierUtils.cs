...
namespace BezierSolution
{
	public static class BezierUtils
	{
		...
		private static Color SPLINE_GIZMO_COLOR = new Color( 0.8f, 0.6f, 0.4f );
		...
		static void DrawSplineGizmo( BezierSpline spline, GizmoType gizmoType )
		{
			...
			Vector3 lastInfluencePos0 = GetInfluencePos( 0, lastPos, spline, 1 );
			Vector3 lastInfluencePos1 = GetInfluencePos( 0, lastPos, spline, -1 );
			...
			for( float i = increaseAmount; i < 1f; i += increaseAmount )
			{
				...
				var influencePos0 = GetInfluencePos( i, pos, spline, 1 );
				var influencePos1 = GetInfluencePos( i, pos, spline, -1 );

				Gizmos.color = Color.grey;
				Gizmos.DrawLine( lastInfluencePos0, influencePos0 );
				Gizmos.DrawLine( lastInfluencePos1, influencePos1 );
				lastInfluencePos0 = influencePos0;
				lastInfluencePos1 = influencePos1;
			}

			var endPos = spline.GetPoint( 1 );
			Gizmos.DrawLine( lastInfluencePos0, GetInfluencePos( 1, endPos, spline, 1 ) );
			Gizmos.DrawLine( lastInfluencePos1, GetInfluencePos( 1, endPos, spline, -1 ) );

			Gizmos.color = SPLINE_GIZMO_COLOR;
			Gizmos.DrawLine( lastPos, endPos );
		}
