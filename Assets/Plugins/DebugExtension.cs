...
public static class Draw {
	...
	static DrawParameters activeParameters = new DrawParameters( defaultColor, 0, true );
	static Stack<DrawParameters> parameterStack = new Stack<DrawParameters>();

	public static void PushParams () {
		parameterStack.Push( activeParameters );
		activeParameters = new DrawParameters( defaultColor, 0, true );
	}
	public static void PushParams ( Color color ) {
		parameterStack.Push( activeParameters );
		activeParameters = new DrawParameters( color, 0, true );
	}
	public static void PushParams ( float duration ) {
		parameterStack.Push( activeParameters );
		activeParameters = new DrawParameters( defaultColor, duration, true );
	}
	public static void PushParams ( Color color, float duration ) {
		parameterStack.Push( activeParameters );
	...
	public static void PopParams () {
		if( parameterStack.Count > 0 ) activeParameters = parameterStack.Pop();
	}
	...
	public static void Ray ( Vector3 origin, Vector3 direction ) {
		Debug.DrawRay( origin, direction, activeParameters.color, activeParameters.duration, activeParameters.depthTest );
	}
	...
	public static void Line ( Vector3 start, Vector3 end ) {
		Debug.DrawLine( start, end, activeParameters.color, activeParameters.duration, activeParameters.depthTest );
	}
	...
	public static void Circle ( Vector3 center, float radius, Vector3 forward, Color color, int numSegments = defaultCircleSegmentCount ) {
		PushParams( color );
		Circle( center, radius, forward, numSegments );
		PopParams();
	}
	...
	// ARROW METHODS //
	// -- incomplete
	public static void Arrow ( Vector3 position, Vector3 direction, Color color, float angle = 45 ) {
		PushParams( color );
		Arrow( position, direction, angle );
		PopParams();
	}
